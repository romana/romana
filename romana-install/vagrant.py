...
if options.list:
    ...
    for host in ssh_config:
        ...
        stack_nodes.append(host)
        if host.endswith('-controller'):
            controller.append(host)
            meta['hostvars'][host][_stack_host] = 'Controller'
        elif host[:-2].endswith('-compute'):
            computes.append(host)
            meta['hostvars'][host][_stack_host] = 'Compute' + host[-2:]

    print(json.dumps({_stack_nodes: stack_nodes,
        _controller: controller,
        _computes: computes,
        '_meta': meta}))
    sys.exit(0)
