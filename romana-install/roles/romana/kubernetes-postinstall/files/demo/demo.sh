...
run "get_pods"
desc "Log into nginx-frontend, check if we can reach the backend"
run "kubectl exec nginx-frontend -t -i -- /bin/bash"
desc "Now let's add a policy that permits frontend to connect to the backend"
run "romana add-policy romana-np-frontend-to-backend.json"
desc "Log into nginx-frontend, check if we can reach the backend"
run "kubectl exec nginx-frontend -t -i -- /bin/bash"
